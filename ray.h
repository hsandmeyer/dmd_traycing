...
public:
  ...
  DiffractionPoint(const float x0, const float x1, const float x2,
                   const float amplitude = 1, const float phase_shift = 0)
      : cpplap::Vect<float>(x0, x1, x2), _amplitude(amplitude),
        _phase_shift(phase_shift) {}

  DiffractionPoint(const cpplap::Vect<float> vect, float amplitude = 1,
                   float phase_shift = 0)
      : cpplap::Vect<float>(vect), _amplitude(amplitude),
        _phase_shift(phase_shift) {}
...
bool cmp_rel(const DiffractionPoint &a, const DiffractionPoint &b) {
  return cmp_rel(static_cast<const cpplap::Vect<float> &>(a),
                 static_cast<const cpplap::Vect<float> &>(b)) &&
         cpplap::cmp_rel(a._amplitude, b._amplitude) &&
         cpplap::cmp_rel(a._phase_shift, b._phase_shift);
}

std::ostream &operator<<(std::ostream &stream, const DiffractionPoint &a) {
  stream << static_cast<const cpplap::Vect<float> &>(a)
         << "amplitude = " << a._amplitude << std::endl
         << "phase_shift = " << a._phase_shift << std::endl;
  return stream;
}

bool operator==(const DiffractionPoint &a, const DiffractionPoint &b) {
  return (static_cast<const cpplap::Vect<float> &>(a) ==
          static_cast<const cpplap::Vect<float> &>(b)) &&
...
public:
  Ray(const cpplap::Vect<float> r, const cpplap::Vect<float> v,
      const float amplitude = 1)
      : cpplap::Line<float>(r, v), _amplitude(amplitude) {}

  Ray(const cpplap::Line<float> line, const float amplitude = 1)
      : cpplap::Line<float>(line), _amplitude(amplitude) {}
};
...
public:
  Mirror(const cpplap::Vect<float> r, const cpplap::Vect<float> u,
         const cpplap::Vect<float> v, const float amplitude = 1)
  ...
  bool rayInterSec(const Ray ray, DiffractionPoint &diff_point) {
    ...
    diff_point._phase_shift = ray.dist(diff_point);
